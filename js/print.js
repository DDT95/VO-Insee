(function () {
  ...
  const app = openerWindow && openerWindow.voInseeApp;
  const statusEl = document.getElementById("pdfStatus");
  const previewMode = new URLSearchParams(location.search).get("preview") === "1";
  if (!app) {
    document.body.innerHTML = '<p style="padding:40px;font:16px Marianne,Arial,sans-serif">Cette page s’ouvre depuis le bouton « Imprimer la carte » du Portrait Insee.</p>';
    return;
  }
  ...
  document.getElementById("printTitle").textContent = layerDef ? layerDef.label : `${theme.label} dans le Val-d'Oise`;
  document.getElementById("printSubtitle").textContent = `${theme.label} · ${territoryType === "EPCI" ? "Lecture par EPCI" : "Lecture communale"}${selectedName ? ` · ${selectedName} sélectionné` : ""}`;
  const today = new Date().toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" });
  document.getElementById("printSources").innerHTML = `<span class="src-line">Sources : Insee, Base du dossier complet (RP, Filosofi, REE, BPE) · réutilisation val-doise-logement-habitat & domicile-travail</span><span class="src-line">Auteur : DDT 95</span><span class="src-line">Date : ${today}</span>`;
